-- תצוגת אדמין: פונקציה נפרדת בלי עדכון card_opened_at (PostgREST לעיתים לא מכיר overload של get_public_ticket(text, boolean))

DROP FUNCTION IF EXISTS public.get_public_ticket(text, boolean);
...
COMMENT ON FUNCTION public.get_public_ticket_no_open_record(text) IS
  'כרטיס ציבורי לתצוגת אדמין — ללא עדכון card_opened_at (קריאה מ־?staff_preview=1)';

GRANT EXECUTE ON FUNCTION public.get_public_ticket(text) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_public_ticket_no_open_record(text) TO anon, authenticated;
