-- רק שני סטטוסים: pending (לא נכנס), entered (נכנס) — ביטול blocked

UPDATE public.guests SET status = 'pending' WHERE status = 'blocked';

ALTER TABLE public.guests ALTER COLUMN status DROP DEFAULT;
ALTER TABLE public.guests ALTER COLUMN status TYPE text USING status::text;

DROP TYPE public.guest_status;

CREATE TYPE public.guest_status AS ENUM ('pending', 'entered');

ALTER TABLE public.guests
  ALTER COLUMN status TYPE public.guest_status USING status::public.guest_status;

ALTER TABLE public.guests
  ALTER COLUMN status SET DEFAULT 'pending'::public.guest_status;
