-- פתיחת כרטיס עם ?staff_preview=1 (אדמין) — ללא עדכון card_opened_at

DROP FUNCTION IF EXISTS public.get_public_ticket(text);

CREATE OR REPLACE FUNCTION public.get_public_ticket(p_code text, p_record_open boolean DEFAULT true)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
COMMENT ON FUNCTION public.get_public_ticket(text, boolean) IS
  'כרטיס ציבורי; p_record_open=false לתצוגת אדמין (?staff_preview=1) בלי לסמן שאורח פתח את הדף';

GRANT EXECUTE ON FUNCTION public.get_public_ticket(text, boolean) TO anon, authenticated;
